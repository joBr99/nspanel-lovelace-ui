...
ReleaseNotes:
    ...
    See ChangeLog all Release Notes: https://github.com/joBr99/nspanel-lovelace-ui/wiki/Release-Notes
        ...
        - 06.10.2024 - v4.4.0.7  Fix: first start and initialisation with new NSPanel device - Fix by wolwin
...
async function Init_Release() {
    ...
    try {
        if (existsObject(NSPanel_Path + 'Display_Firmware.desiredVersion') == false) {
            await createStateAsync(NSPanel_Path + 'Display_Firmware.desiredVersion', String(desired_display_firmware_version), { type: 'string', write: false });
        } else {
            await setStateAsync(NSPanel_Path + 'Display_Firmware.desiredVersion', String(desired_display_firmware_version), true);
        }
...
async function InitPageNavi() {
    try {
        if (!existsState(NSPanel_Path + 'PageNavi')) {
            ...
            await setStateAsync(NSPanel_Path + 'PageNavi', <iobJS.State>{ val: String( { pagetype: 'page', pageId: 0 } ), ack: true });
        }
    } catch (err: any) {
