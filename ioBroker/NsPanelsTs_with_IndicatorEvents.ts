...
const NSPanel_Path = '0_userdata.0.NSPanels.Simulation.';         // Anpassen an das jewilige NSPanel
const NSPanel_Alarm_Path = '0_userdata.0.NSPanels.';     // Pfad für gemeinsame Nutzung durch mehrere Panels (bei Nutzung der cardAlarm)
...
let Level_0_Grundstueck = <PageGrid>
{
    ...
    'useColor': true,
    'items': [
       // <PageItem>{ navigate: true, id: 'alias.0.Haus.Indikator', targetPage: 'Level_1_Haus', name: 'Haus' ,icon: 'home', offColor: Menu, onColor: MenuHighInd},
       ...
       // <PageItem>{ navigate: true, id: 'alias.0.Garage.Indikator', targetPage: 'Level_1_Garage', name: 'Garage' ,icon: 'garage-variant', offColor: Menu, onColor: MenuHighInd},
...
export const config = <Config> {
    panelRecvTopic: 'mqtt.4.SmartHome.tasmota_B8CAC0.tele.RESULT',       // Bitte anpassen
    panelSendTopic: 'mqtt.4.SmartHome.tasmota_B8CAC0.cmnd.CustomSend',   // Bitte anpassen

    leftScreensaverEntity:
        [
            // leftScreensaverEntity 1 (only Advanced Screensaver)
            {
                ScreensaverEntity: AliasPath +'Sensor.AHT2X.Temperature.ACTUAL',
                ScreensaverEntityFactor: 1,
                ScreensaverEntityDecimalPlaces: 1,
                ScreensaverEntityIconOn: 'thermometer',
                ScreensaverEntityIconOff: null,
                ScreensaverEntityText: 'Ist-Temperatur',
            ...
            {
                ScreensaverEntity: AliasPath +'Sensor.AHT2X.Humidity.ACTUAL',
                ScreensaverEntityFactor: 1,
                ScreensaverEntityDecimalPlaces: 1,
                ScreensaverEntityIconOn: 'water-percent',
                ScreensaverEntityIconOff: null,
                ScreensaverEntityText: "Luftfeuchte",
                ScreensaverEntityUnitText: '%',
                ScreensaverEntityIconColor: {'val_min': 0, 'val_max': 100}
            },
            // leftScreensaverEntity 3 (only Advanced Screensaver)
        	{
                ScreensaverEntity: 'alias.0.Thermostate.Wohn.SET',
                ScreensaverEntityFactor: 1,
                ScreensaverEntityDecimalPlaces: 1,
                ScreensaverEntityIconOn: 'thermostat',
                ScreensaverEntityIconOff: null,
                ScreensaverEntityText: 'Soll-Temperatur',
                ScreensaverEntityUnitText: '°C',
                ScreensaverEntityIconColor: {'val_min': 0, 'val_max': 35, 'val_best': 22}
            },
        ],
    ...
    bottomScreensaverEntity :  
        [
            ...
            {
                ScreensaverEntity: 'alias.0.WetterStation.Aktuell.PRECIPITATION_CHANCE',
                ScreensaverEntityFactor: 1,
            ...
            {
                ScreensaverEntity: 'alias.0.WetterStation.Aktuell.WIND_SPEED',
                ScreensaverEntityFactor: 1,
                ScreensaverEntityDecimalPlaces: 0,
                ScreensaverEntityIconOn: 'weather-windy',
                ScreensaverEntityIconOff: null,
                ScreensaverEntityText: "Wind",
                ScreensaverEntityUnitText: 'bft',
                ScreensaverEntityIconColor: { 'val_min': 0, 'val_max': 12 }
            },
            // bottomScreensaverEntity 3
            {
                ScreensaverEntity: 'alias.0.WetterStation.Aktuell.WIND_GUST',
            ...
            {
                ScreensaverEntity: 'alias.0.WetterStation.Aktuell.WIND_DIRECTION',
                ScreensaverEntityFactor: 1,
                ScreensaverEntityDecimalPlaces: 0,
            ...
            {
                ScreensaverEntity: 'alias.0.WetterStation.Aktuell.HUMIDITY',
                ScreensaverEntityFactor: 1,
                ScreensaverEntityDecimalPlaces: 1,
                ScreensaverEntityIconOn: 'water-percent',
                ScreensaverEntityIconOff: null,
            ...
            {
                ScreensaverEntity: 'alias.0.WetterStation.Aktuell.UV',
                ScreensaverEntityFactor: 1,
                ScreensaverEntityDecimalPlaces: 0,
    ...
    indicatorScreensaverEntity:
        [
            ...
            { 
                ScreensaverEntity: 'worx.0.202230267204003017D5.mower.state',
                ScreensaverEntityFactor: 1,
                ScreensaverEntityDecimalPlaces: 0,
    ...
    mrIcon1ScreensaverEntity: 
        { 
            ...
            ScreensaverEntityIconOn: null, //'lightbulb',                           //Rename
            ScreensaverEntityIconOff: null, 
            ScreensaverEntityValue: null, //'0_userdata.0.trashschedule.0.next.type',                                   //New
    ...
    button1Page: null, //button1Page,   //Beispiel-Seite auf Button 1, wenn Rule2 definiert - Wenn nicht definiert --> button1Page: null, 
    button2Page: null //button2Page    //Beispiel-Seite auf Button 2, wenn Rule2 definiert - Wenn nicht definiert --> button1Page: null,
};
            ...
            if (weatherAdapterInstance == 'daswetter.0.') {
                ...
                try {
                    if (!existsState(config.weatherEntity + '.ICON') && existsState('accuweather.0.Current.WeatherIcon')) {
                        ...
                        await createAliasAsync(config.weatherEntity + '.TEMP', 'mqtt.0.Hzg.AussenTemp', true, <iobJS.StateCommon>{ type: 'number', role: 'value.temperature', name: 'TEMP' });
...
async function SendToPanel(val: Payload | Payload[]) {
    try {
        if (Array.isArray(val)) {
            val.forEach(async function (id) {
                await setStateAsync(config.panelSendTopic, id.payload);
                if (Debug) {
                    console.log(id.payload);
                }
            });
        } else {
            await setStateAsync(config.panelSendTopic, val.payload);
        }
...
function HandleButtonEvent(words: any): void {
    try {
        ...
        // gargano
        if (words[2] == 'f1Icon' ||words[2] == 'f2Icon' || words[2] == 'f3Icon' || words[2] == 'f4Icon') {
            buttonAction = words[2];
        }
...
on({ id: config.panelRecvTopic.substring(0, config.panelRecvTopic.length - 'RESULT'.length) + 'SENSOR' }, async (obj) => {
    try {
        ...
        // digital temp and rf
        await createStateAsync(NSPanel_Path + 'Sensor.AHT2X.Temperature', <iobJS.StateCommon>{ type: 'number', 'unit': '°C' });
         await createStateAsync(NSPanel_Path + 'Sensor.AHT2X.Humidity', <iobJS.StateCommon>{ type: 'number', 'unit': '%' });
        ...
        try {
            ...
            // digital temp and rf
            await setStateAsync(NSPanel_Path + 'Sensor.AHT2X.Temperature', <iobJS.State>{ val: parseFloat(Tasmota_Sensor.AHT2X.Temperature), ack: true });
            await setStateAsync(NSPanel_Path + 'Sensor.AHT2X.Humidity', <iobJS.State>{ val: parseFloat(Tasmota_Sensor.AHT2X.Humidity), ack: true });
        ...
        if (autoCreateAlias) {
            ...
            // digital temp and rf
            await createAliasAsync(AliasPath + 'Sensor.AHT2X.Temperature.ACTUAL', NSPanel_Path + 'Sensor.AHT2X.Temperature', true, <iobJS.StateCommon>{ type: 'number', 'unit': '°C' });
            await createAliasAsync(AliasPath + 'Sensor.AHT2X.Humidity.ACTUAL', NSPanel_Path + 'Sensor.AHT2X.Humidity', true, <iobJS.StateCommon>{ type: 'number', 'unit': '%' });
